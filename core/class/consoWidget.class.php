...
class consoWidget extends eqLogic {
	...
	public function getCode(){
		...
		if (config::byKey('ProtocoleURL', 'consoWidget') == 1 and filter_var($ip, FILTER_VALIDATE_IP) == 0) {
			...
			$rech = 'external';
		} elseif (config::byKey('ProtocoleURL', 'consoWidget') == 0 and (filter_var($ip, FILTER_VALIDATE_IP) or $ip != config::byKey('externalAddr'))) {
		...
		if (config::byKey('modeDebug', 'consoWidget') == 1) {
			$code = '<div width="100%" height="10%" style="background-color: #262626; color: #acacac;">           DEBUG 1: '. $chemin.'/index.php?v=d&m=consoWidget&p=widget&id='.$this->getConfiguration('idequip').'&widget='.$this->getConfiguration('type_consoWidget').' IP: '.$ip.' Adresse interne: '.config::byKey('internalAddr').' validate_ip: '.$validate_ip.' rech:'.$rech.'</div>
